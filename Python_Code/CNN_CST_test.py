...
with open('ScatterMat.csv', 'r') as f:
    reader = csv.reader(f)
    data = list(reader)
...
ScatMatTorch = np_to_00torch(data_array).float()
...
cv2disp("ScatterGram", torch_to_np(ScatMatTorch), disp_scale*nxd, 0, disp_scale)
...
class FBP_CNN_Net(nn.Module):
    ...
    def forward(self, sino_for_reconstruction):
        ...
        cv2disp("Filtered ScatterGram", torch_to_np(filtered_sino), disp_scale*(nxd+nrd), 0, disp_scale)
        cv2disp("FBP", torch_to_np(recon), disp_scale*(nxd+nrd), disp_scale*nphi+30, disp_scale)
        cv2disp("FP Scatter", torch_to_np(fpsino), disp_scale*(nxd+nrd+nphi), 0, disp_scale)
...
for ep in range(epochs):
    fbp_recon, rec_fp = fbpnet(ScatMatTorch)
    loss = loss_fun(rec_fp, torch.squeeze(ScatMatTorch))
