...
set.seed(222) 
nn$weights[[1]] <- matrix(runif(1,min=-1,max=1), nrow = 16, ncol = 2)
nn$weights[[2]] <- matrix(runif(1,min=-1,max=1), nrow = 3, ncol = 1)
...
write_csv(filt_sinoA, "raw_data/filtsinoa-KW.csv")
...
write_csv(filt_sinoB, "raw_data/filtsinob-KW.csv")


write_csv(m1, "raw_data/m1-KW.csv")
write_csv(m2, "raw_data/m2-KW.csv")
...
```{r}
 p <- read.csv("C:/Users/capa01/Documents/R-Projects/ANN_work/clean_data/FBP1-KW.csv")
p <- p %>% as.matrix()
image(p)
```
